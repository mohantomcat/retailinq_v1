...
public class CheckpointRepository {
    ...
    public SiocsPollCheckpoint findOrCreate(String pollerId, String tenantId) {
        try {
            return findById(pollerId);
        } catch (EmptyResultDataAccessException e) {
            Timestamp epoch = Timestamp.from(Instant.EPOCH);
            String insertSql = """
                    INSERT INTO recon.siocs_poll_checkpoint (
    ...
    public SiocsPollCheckpoint findById(String pollerId) {
        String sql = """
                SELECT poller_id, tenant_id,
                       ...
                       lock_owner,
                       lock_expires_at
                FROM recon.siocs_poll_checkpoint
                WHERE poller_id = ?
                """;
        return jdbcTemplate.queryForObject(sql,
                (rs, rn) -> SiocsPollCheckpoint.builder()
                        .pollerId(rs.getString("poller_id"))
                        .tenantId(rs.getString("tenant_id"))
                        .lastProcessedTimestamp(
                                rs.getTimestamp("last_processed_timestamp"))
                        .lastProcessedExternalId(
                                rs.getString("last_processed_external_id"))
                        .lastProcessedId(
                                rs.getObject("last_processed_id", Long.class))
                        .lastPollStartedAt(
                                rs.getTimestamp("last_poll_started_at"))
                        .lastPollCompletedAt(
                                rs.getTimestamp("last_poll_completed_at"))
                        .lastPollStatus(
                                rs.getString("last_poll_status"))
                        .lastErrorMessage(rs.getString("last_error_message"))
                        .totalRecordsPolled(
                                rs.getLong("total_records_polled"))
    ...
    public boolean tryAcquireLease(String pollerId,
        ...
        String sql = """
                UPDATE recon.siocs_poll_checkpoint
                SET lock_owner = ?,
    ...
    public void markCompleted(String pollerId,
        ...
        String sql = """
                ...
                """;
        Timestamp now = Timestamp.from(Instant.now());
        jdbcTemplate.update(sql, now, recordsPolled,
                now, pollerId);
    }
    ...
    public void markFailed(String pollerId,
        ...
        String sql = """
                ...
                """;
        Timestamp now = Timestamp.from(Instant.now());
        jdbcTemplate.update(sql, now, errorMessage,
                now, pollerId);
    }
