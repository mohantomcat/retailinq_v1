...
class CloudRestDownloadServiceTest {
    ...
    void setUp() {
        ...
        lenient().when(runtimeConfigService.getBoolean("MFCS_CONNECTOR_ENABLED", true)).thenReturn(true);
        lenient().when(runtimeConfigService.getInt("MFCS_OVERLAP_MINUTES", 10)).thenReturn(10);
        lenient().when(runtimeConfigService.getInt("MFCS_BATCH_SIZE", 100)).thenReturn(100);
    ...
    void runDownloadCyclePersistsAndAdvancesCheckpoint() {
        ...
        when(restClient.fetchTransactions(any(), any(), eq(100)))
                .thenReturn(CloudApiPage.builder()
                        ...
                        .hasMore(false)
                        .build());
        when(rawRepository.insert(eq("tenant-india"), eq("MFCS_SIM"), eq("src-1"),
                eq("0"), any(), any())).thenReturn(1L);

        service.runDownloadCycle();

        verify(checkpointRepository).markStarted("cloud-rest-main");
        verify(rawRepository).insert(eq("tenant-india"), eq("MFCS_SIM"), eq("src-1"),
                eq("0"), any(), any());
        verify(transactionRepository).upsertTransactionRows(
                eq("tenant-india"), eq("MFCS_SIM"), eq("0"), eq(1L), any());
