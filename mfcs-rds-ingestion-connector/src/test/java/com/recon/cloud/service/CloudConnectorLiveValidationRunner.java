...
public class CloudConnectorLiveValidationRunner {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.setProperty("mfcs.connector.enabled", "true");
        System.setProperty("mfcs.connector.scheduler-enabled", "false");
        ...
        try (ConfigurableApplicationContext context =
            ...
            long rawBefore = queryLong(jdbcTemplate,
                    "SELECT COUNT(*) FROM recon.mfcs_ingestion_raw");
            long txnBefore = queryLong(jdbcTemplate,
                    "SELECT COUNT(*) FROM recon.mfcs_ingestion_transaction");
            ...
            long nullExternalIdAfterDownload = queryLong(jdbcTemplate,
                    "SELECT COUNT(*) FROM recon.mfcs_ingestion_transaction WHERE external_id IS NULL OR BTRIM(external_id) = ''");
            long literalNullExternalIdAfterDownload = queryLong(jdbcTemplate,
                    "SELECT COUNT(*) FROM recon.mfcs_ingestion_transaction WHERE LOWER(BTRIM(COALESCE(external_id, ''))) = 'null'");
    ...
    private static void resetStagingTables(JdbcTemplate jdbcTemplate) {
        jdbcTemplate.execute(
                "TRUNCATE TABLE recon.mfcs_ingestion_error, " +
                        "recon.mfcs_ingestion_transaction, " +
                        "recon.mfcs_ingestion_raw, " +
                        "recon.mfcs_ingestion_checkpoint RESTART IDENTITY CASCADE");
    }
}
